...
void GaussSeidel3D(MatrixS *pMat)
{
	...
	for(k=ks; k<=ke; k++)
		for(j=js; j<=je; j++)
			...
			theta[k][j][i][7] = Crat * hdtodx1 * (Ci0 + Ci1) - Eratio * pMat->dt * (Sigma_aF - Sigma_sF) * velocity_x;
			theta[k][j][i][8] = Crat * hdtodx2 * (Cj0 + Cj1) - Eratio * pMat->dt * (Sigma_aF - Sigma_sF) * velocity_y;
			theta[k][j][i][9] = Crat * hdtodx3 * (Ck0 + Ck1) - Eratio * pMat->dt * (Sigma_aF - Sigma_sF) * velocity_z;
