...
for(n=0; n<Ncycle; n++){
	...
	for(k=ks; k<=ke; k++)
		...
		if(n == 0){
			...
			RHS[0] = pMat->U[k][j][i].Er + Crat * dt * Sigma_a * T4;
			RHS[1] = pMat->U[k][j][i].Fr1 + dt * Sigma_a * T4 * velocity_x;
			RHS[2] = pMat->U[k][j][i].Fr2 + dt * Sigma_a * T4 * velocity_y;
			RHS[3] = pMat->U[k][j][i].Fr3 + dt * Sigma_a * T4 * velocity_z;
