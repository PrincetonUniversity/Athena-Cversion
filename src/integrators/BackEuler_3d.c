			...
			for(k=ks-nghost; k<=ke+nghost;k++){
				for(j=js-nghost; j<=je+nghost; j++){
					for(i=is-nghost; i<=ie+nghost; i++){
						...
						dErdx_t0[k][j][i] = -pG->U[k][j][i].Sigma_t * pG->U[k][j][i].Fr1;
						dErdy_t0[k][j][i] = -pG->U[k][j][i].Sigma_t * pG->U[k][j][i].Fr2;
						dErdz_t0[k][j][i] = -pG->U[k][j][i].Sigma_t * pG->U[k][j][i].Fr3;
					}
				}
			}	
			t0flag = 0;
...
*/
		...
		/* index of the vector should be the global vector, not the partial vector */	
    		tempvalue   = pG->U[k][j][i].Er + Crat * dt * Sigma_a * T4;
		...
		if(bgflag){
			...
			tempvalue += dt * (Sigma_a - Sigma_s) * (velocity_x * Fr0x + velocity_y * Fr0y + velocity_z * Fr0z);
		}	
		...
		/*----------------------------*/
    		tempvalue = pG->U[k][j][i].Fr1 + dt * Sigma_a * T4 * velocity_x;
		...
		/*-------------------------*/
		tempvalue = pG->U[k][j][i].Fr2 + dt * Sigma_a * T4 * velocity_y;
		...
		/*-------------------------*/
		tempvalue = pG->U[k][j][i].Fr3 + dt * Sigma_a * T4 * velocity_z;
	...
	for(k=ks; k<=ke; k++){
		for(j=js; j<=je; j++){
			...
			theta[6] = 1.0 + Crat * hdtodx1 * (2.0 + Ci1 - Ci0) * sqrt(pG->U[k][j][i].Edd_11) 
				...
				+ Crat * pG->dt * Sigma_a 
				+ pG->dt * (Sigma_a - Sigma_s) * ((1.0 + pG->U[k][j][i].Edd_11) * velocity_x 
				+ velocity_y * pG->U[k][j][i].Edd_21 + velocity_z * pG->U[k][j][i].Edd_31) * velocity_x / Crat
				+ pG->dt * (Sigma_a - Sigma_s) * ((1.0 + pG->U[k][j][i].Edd_22) * velocity_y 
				+ velocity_x * pG->U[k][j][i].Edd_21 + velocity_z * pG->U[k][j][i].Edd_32) * velocity_y / Crat
				+ pG->dt * (Sigma_a - Sigma_s) * ((1.0 + pG->U[k][j][i].Edd_33) * velocity_z 
				+ velocity_x * pG->U[k][j][i].Edd_31 + velocity_y * pG->U[k][j][i].Edd_32) * velocity_z / Crat;
			theta[7] = Crat * hdtodx1 * (Ci0 + Ci1)	- pG->dt * (Sigma_a - Sigma_s) * velocity_x;
			theta[8] = Crat * hdtodx2 * (Cj0 + Cj1)	- pG->dt * (Sigma_a - Sigma_s) * velocity_y;
			theta[9] = Crat * hdtodx3 * (Ck0 + Ck1)	- pG->dt * (Sigma_a - Sigma_s) * velocity_z;
			theta[10] = -Crat * hdtodx1 * (1.0 - Ci1) * sqrt(pG->U[k][j][i+1].Edd_11);
			theta[11] = Crat * hdtodx1 * (1.0 - Ci1);
			theta[12] = -Crat * hdtodx2 * (1.0 - Cj1) * sqrt(pG->U[k][j+1][i].Edd_22);
			theta[13] = Crat * hdtodx2 * (1.0 - Cj1);
			...
			phi[6] = Crat * hdtodx1 * (Ci0 + Ci1) * pG->U[k][j][i].Edd_11
			       ...
			       - pG->dt * Sigma_t * ((1.0 + pG->U[k][j][i].Edd_11) * velocity_x + pG->U[k][j][i].Edd_21 * velocity_y + pG->U[k][j][i].Edd_31 * velocity_z) 
			...
			psi[6] = Crat * hdtodx1 * (Ci0 + Ci1) * pG->U[k][j][i].Edd_21
			       ...
			       - pG->dt * Sigma_t * ((1.0 + pG->U[k][j][i].Edd_22) * velocity_y + pG->U[k][j][i].Edd_21 * velocity_x + pG->U[k][j][i].Edd_32 * velocity_z) 
			...
			varphi[6] = Crat * hdtodx1 * (Ci0 + Ci1) * pG->U[k][j][i].Edd_31
			       ...
			       - pG->dt * Sigma_t * ((1.0 + pG->U[k][j][i].Edd_33) * velocity_z + pG->U[k][j][i].Edd_31 * velocity_x + pG->U[k][j][i].Edd_32 * velocity_y) 
			       + pG->dt * Sigma_a * velocity_z;
...
/* debug node */
	...
	for(k=ks;k<=ke;k++){	
		for(j=js;j<=je;j++){
			...
			lis_vector_get_value(INIguess,4*(k-ks)*Nx*Ny + 4*(j-js)*Nx + 4*(i-is) + count_Grids,&(pG->U[k][j][i].Er));
			lis_vector_get_value(INIguess,4*(k-ks)*Nx*Ny + 4*(j-js)*Nx + 4*(i-is) + 1 + count_Grids,&(pG->U[k][j][i].Fr1));
			lis_vector_get_value(INIguess,4*(k-ks)*Nx*Ny + 4*(j-js)*Nx + 4*(i-is) + 2 + count_Grids,&(pG->U[k][j][i].Fr2));
			lis_vector_get_value(INIguess,4*(k-ks)*Nx*Ny + 4*(j-js)*Nx + 4*(i-is) + 3 + count_Grids,&(pG->U[k][j][i].Fr3));
/*
...
void BackEuler_init_3d(const int Nx, const int Ny, const int Nz, int NGridx, int NGridy, int NGridz)
{
...
/* The matrix Euler is stored as a compact form.  */
	...
	if((Er_t0 = (Real***)calloc_3d_array(Nz+2*nghost, Ny+2*nghost, Nx+2*nghost, sizeof(Real))) == NULL)
		ath_error("[BackEuler_init_2d]: malloc returned a NULL pointer\n");
	if((dErdx_t0 = (Real***)calloc_3d_array(Nz+2*nghost, Ny+2*nghost, Nx+2*nghost, sizeof(Real))) == NULL)
		ath_error("[BackEuler_init_2d]: malloc returned a NULL pointer\n");
	if((dErdy_t0 = (Real***)calloc_3d_array(Nz+2*nghost, Ny+2*nghost, Nx+2*nghost, sizeof(Real))) == NULL)
		ath_error("[BackEuler_init_2d]: malloc returned a NULL pointer\n");
	if((dErdz_t0 = (Real***)calloc_3d_array(Nz+2*nghost, Ny+2*nghost, Nx+2*nghost, sizeof(Real))) == NULL)
		ath_error("[BackEuler_init_2d]: malloc returned a NULL pointer\n");
	if((Fr1_t0 = (Real***)calloc_3d_array(Nz+2*nghost, Ny+2*nghost, Nx+2*nghost, sizeof(Real))) == NULL)
		ath_error("[BackEuler_init_2d]: malloc returned a NULL pointer\n");
	if((Fr2_t0 = (Real***)calloc_3d_array(Nz+2*nghost, Ny+2*nghost, Nx+2*nghost, sizeof(Real))) == NULL)
		ath_error("[BackEuler_init_2d]: malloc returned a NULL pointer\n");
	if((Fr3_t0 = (Real***)calloc_3d_array(Nz+2*nghost, Ny+2*nghost, Nx+2*nghost, sizeof(Real))) == NULL)
		ath_error("[BackEuler_init_2d]: malloc returned a NULL pointer\n");
...
/* variables used to subtract background state */
	...
	free_3d_array(dErdz_t0);	
	free_3d_array(Fr1_t0);
	free_3d_array(Fr2_t0);
	free_3d_array(Fr3_t0);
...
void ie_je_k_phy_phy(const int k)
{
	...
	indexi0Fr2 = NoFr2 + 4 + count1y + count1y;
