                   ...
                   const int il, const int iu, PVar1DS Wl[], PVar1DS Wr[])
{
  ...
  Real dW[NWAVE+NSCALARS],W6[NWAVE+NSCALARS];
  Real *pWl, *pWr;
  Real delta,ql,qr,zc,zr,zl,q1,q2,gamma;
  Real qx1,qxx1,qx2,qxx2;
 ...
 * cell-centered vals */
    ...
    for (n=0; n<(NWAVE+NSCALARS); n++) {
      qa = (Wrv[n]-pW[i][n])*(pW[i][n]-Wlv[n]);
      qb = Wrv[n]-Wlv[n];
      qc = 6.0*(pW[i][n] - 0.5*(Wlv[n]*(1.0-gamma) + Wrv[n]*(1.0+gamma)));
      if (qa <= 0.0) {  
        /* IF a_i IS A LOCAL EXTREMUM, FLATTEN THE PARABOLA */
        Wlv[n] = pW[i][n];
        Wrv[n] = pW[i][n];
      } else if ((qb*qc) > (qb*qb)) {  
        Wlv[n] = (6.0*pW[i][n] - Wrv[n]*(4.0+3.0*gamma))/(2.0-3.0*gamma);
      } else if ((qb*qc) < -(qb*qb)) {
        Wrv[n] = (6.0*pW[i][n] - Wlv[n]*(4.0-3.0*gamma))/(2.0+3.0*gamma);
      }
    }
...
/* dW IS THE BIG DELTA */
    ...
    for (n=0; n<(NWAVE+NSCALARS); n++) {
      dW[n] = Wrv[n] - Wlv[n];
      W6[n] = 6.0*(pW[i][n] - 0.5*(Wlv[n]*(1.0-gamma) + Wrv[n]*(1.0+gamma)));
    }
 ...
 */
    ...
    qxx1 = SQR(qx1)*delta/(3.0*(ri[i+1]-delta*qx1));
    for (n=0; n<(NWAVE+NSCALARS); n++) {
    ...
    qxx2 = SQR(qx2)*delta/(3.0*(ri[i]+delta*qx2));
    for (n=0; n<(NWAVE+NSCALARS); n++) {
 ...
 */
    ...
    for (n=0; n<NWAVE; n++) {
      if (ev[n] >= 0.) {
        ...
        qx2  = 0.5*dtodx*ev[n];
        qxx1 = SQR(qx1)*delta/(3.0*(ri[i+1]-delta*qx1));
        qxx2 = SQR(qx2)*delta/(3.0*(ri[i+1]-delta*qx2));
        for (m=0; m<NWAVE; m++) {
          qa += lem[n][m]*(qx1 *(dW[m]-(1.0-FOUR_3RDS*qx1)*W6[m])
                         - qxx1*(dW[m]-(1.0-      2.0*qx1)*W6[m])
                         - qx2 *(dW[m]-(1.0-FOUR_3RDS*qx2)*W6[m])
                         + qxx2*(dW[m]-(1.0-      2.0*qx2)*W6[m]));
        }
        for (m=0; m<NWAVE; m++) pWl[m] += qa*rem[m][n];
...
/* For HLL fluxes, subtract wave moving away from interface as well. */
        if (ev[n] > 0.0) {
          ...
          qx1  = -0.5*dtodx*MIN(ev[0],0.0);
          qx2  = -0.5*dtodx*ev[n];
          qxx1 = SQR(qx1)*delta/(3.0*(ri[i]+delta*qx1));
          qxx2 = SQR(qx2)*delta/(3.0*(ri[i]+delta*qx2));
          for (m=0; m<NWAVE; m++) {
            qa += lem[n][m]*(-qx1*(dW[m]+(1.0-FOUR_3RDS*qx1)*W6[m])
                           - qxx1*(dW[m]+(1.0-      2.0*qx1)*W6[m])
