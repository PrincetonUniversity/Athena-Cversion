...
void lr_states_cyl(const PVar1DS W[], const Real Bxc[], const Real dtodx,
                   const int il, const int iu, PVar1DS Wl[], PVar1DS Wr[])
{
  int i,n,m;
  Real lim_slope1,lim_slope2,qa,qx;
  ...
  Real dW[NWAVE+NSCALARS],dWm[NWAVE+NSCALARS];
  Real *pWl, *pWr;
  /* ADDITIONAL VARIABLES FOR CYLINDRICAL COORDINATES */
  Real delta,ql,qr,zc,zr,zl,q1,q2,gamma,opg,omg,beta,betai,C;
 ...
 * Note we access contiguous array elements by indexing pointers for speed */
    ...
    for (n=0; n<(NWAVE+NSCALARS); n++) {
      ...
      zc = 1.0/(1.0 - SQR(delta)/(12.0*r[i+1]*r[i-1]));
      zl = 1.0/(1.0 - SQR(delta)/(12.0*r[i  ]*r[i-1]));
      zr = 1.0/(1.0 - SQR(delta)/(12.0*r[i+1]*r[i  ]));
      dWc[n] = (pW[i+1][n] - pW[i-1][n])*zc;
      dWl[n] = (pW[i  ][n] - pW[i-1][n])*zl;
      dWr[n] = (pW[i+1][n] - pW[i  ][n])*zr;
      if (dWl[n]*dWr[n] > 0.0) {
        dWg[n] = 2.0*dWl[n]*dWr[n]/(dWl[n]+dWr[n]);
      } else {
        dWg[n] = 0.0;
 ...
 */
    ...
    qx = 0.5*MAX(ev[NWAVE-1],0.0)*dtodx;
    ql = 1.0 - (delta*qx)/(3.0*(ri[i+1]-delta*qx));
    for (n=0; n<(NWAVE+NSCALARS); n++) {
      pWl[n] = Wrv[n] - qx*dW[n]*ql;
    }

    qx = -0.5*MIN(ev[0],0.0)*dtodx;
    qr = 1.0 + (delta*qx)/(3.0*(ri[i]+delta*qx));
    for (n=0; n<(NWAVE+NSCALARS); n++) {
      pWr[n] = Wlv[n] + qx*dW[n]*qr;
    }
 ...
 */
    ...
    for (n=0; n<NWAVE; n++) {
      if (ev[n] >= 0.) {
        qa = 0.0;
        q1 = 0.5*dtodx*ev[NWAVE-1];
        q2 = 0.5*dtodx*ev[n];
        qx = q1*(1.0 - (delta*q1)/(3.0*(ri[i+1]-delta*q1)))
           - q2*(1.0 - (delta*q2)/(3.0*(ri[i+1]-delta*q2)));
        for (m=0; m<NWAVE; m++) {
          qa += lem[n][m]*qx*dW[m];
        }
...
/* For HLL fluxes, subtract wave moving away from interface as well. */
        if (ev[n] > 0.0) {
          qa = 0.0;
          q1 = -0.5*dtodx*MIN(ev[0],0.0);
          q2 = -0.5*dtodx*ev[n];
          qx = -q1*(1.0 + (delta*q1)/(3.0*(ri[i]+delta*q1)))
              + q2*(1.0 + (delta*q2)/(3.0*(ri[i]+delta*q2)));
          for (m=0; m<NWAVE; m++) {
            qa += lem[n][m]*qx*dW[m];
          }
...
/* For HLL fluxes, subtract wave moving away from interface as well. */
        if (ev[n] < 0.0) {
          qa = 0.0;
          q1 = 0.5*dtodx*MAX(ev[NWAVE-1],0.0);
          q2 = 0.5*dtodx*ev[n];
          qx = q1*(1.0 - (delta*q1)/(3.0*(ri[i+1]-delta*q1)))
             - q2*(1.0 - (delta*q2)/(3.0*(ri[i+1]-delta*q2)));
          for (m=0; m<NWAVE; m++) {
