...
/* Upper and Lower bounds on i,j,k for data dump */
	...
	if (pG->Nx[1] > 1) jo = 1 - nghost; else jo = 0;
	if (pG->Nx[2] > 1) ko = 1 - nghost; else ko = 0;
#endif
...
/* dump_tab_Edd: dump the six components of the Eddington tensor 
*  Only used for RADIATION_HYDRO or RADIATION_MHD
*/
...
/* Write out some header information */
        ...
        fprintf(pfile,"# Eddington tensor at Time = %g, level= %i, domain= %i\n",
          pM->time,nl,nd);
...
/* write out Edd_i column headers */

        fprintf(pfile," [%d]=Edd_11",col_cnt);
        col_cnt++;
        fprintf(pfile," [%d]=Edd_21",col_cnt);
        col_cnt++;
        fprintf(pfile," [%d]=Edd_22",col_cnt);
        col_cnt++;
        fprintf(pfile," [%d]=Edd_31",col_cnt);
        col_cnt++;
	fprintf(pfile," [%d]=Edd_32",col_cnt);
        col_cnt++;
        fprintf(pfile," [%d]=Edd_33",col_cnt);
        col_cnt++;
...
/* Dump all variables */
              ...
              fprintf(pfile,fmt,pG->U[k][j][i].Edd_11);
              fprintf(pfile,fmt,pG->U[k][j][i].Edd_21);
              fprintf(pfile,fmt,pG->U[k][j][i].Edd_22);
              fprintf(pfile,fmt,pG->U[k][j][i].Edd_31);
	      fprintf(pfile,fmt,pG->U[k][j][i].Edd_32);
	      fprintf(pfile,fmt,pG->U[k][j][i].Edd_33);
