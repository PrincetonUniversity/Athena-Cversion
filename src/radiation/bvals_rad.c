 ...
 * FILE: bvals_rad.c
 *
 * PURPOSE: Sets boundary conditions for radiative transfer on each edge of
...
static  int x1cnt=0, x2cnt=0, x3cnt=0; /* # of words passed in x1/x2/x3-dir.*/
#endif /* MPI_PARALLEL */

/*==============================================================================
 * PRIVATE FUNCTION PROTOTYPES:
 *   periodic_???_rad()   - periodic BCs at boundary ???
 *   const_incident_rad() - does nothing
 *   pack_???_rad()       - pack data for MPI non-blocking send at ??? boundary
 *   unpack_???_rad()     - unpack data for MPI non-blocking receive at ??? bdry
 *============================================================================*/

static void periodic_ix1_rad(RadGridS *pRG, int ifr);
static void periodic_ox1_rad(RadGridS *pRG, int ifr);
static void periodic_ix2_rad(RadGridS *pRG, int ifr);
static void periodic_ox2_rad(RadGridS *pRG, int ifr);
static void periodic_ix3_rad(RadGridS *pRG, int ifr);
static void periodic_ox3_rad(RadGridS *pRG, int ifr);

static void const_incident_rad(RadGridS *pRG, int ifr);

static void const_flux_ix1(RadGridS *pRG, int ifr);
static void const_flux_ox1(RadGridS *pRG, int ifr);
static void const_flux_ix2(RadGridS *pRG, int ifr);
static void const_flux_ox2(RadGridS *pRG, int ifr);
static void const_flux_ix3(RadGridS *pRG, int ifr);
static void const_flux_ox3(RadGridS *pRG, int ifr);

static void ProlongateLater(RadGridS *pRG, int ifr);

#ifdef MPI_PARALLEL
static void pack_ix1_rad(RadGridS *pRG, int ifr);
static void pack_ox1_rad(RadGridS *pRG, int ifr);
static void pack_ix2_rad(RadGridS *pRG, int ifr);
static void pack_ox2_rad(RadGridS *pRG, int ifr);
static void pack_ix3_rad(RadGridS *pRG, int ifr);
static void pack_ox3_rad(RadGridS *pRG, int ifr);

static void unpack_ix1_rad(RadGridS *pRG, int ifr);
static void unpack_ox1_rad(RadGridS *pRG, int ifr);
static void unpack_ix2_rad(RadGridS *pRG, int ifr);
static void unpack_ox2_rad(RadGridS *pRG, int ifr);
static void unpack_ix3_rad(RadGridS *pRG, int ifr);
static void unpack_ox3_rad(RadGridS *pRG, int ifr);
#endif /* MPI_PARALLEL */

static void constH_varJ_ix1(RadGridS *pRG, int ifr);

/*=========================== PUBLIC FUNCTIONS ===============================*/

/*----------------------------------------------------------------------------*/
/*! \fn formal_solution(DomainS *pD)
 * Calls approriate boundary condition function for each grid face
 * and handles MPI commmunication.  Modeled after bvals_mhd(). */
...
/* MPI blocks to both left and right */
    if (pRG->rx1_id >= 0 && pRG->lx1_id >= 0) {
      ...
      if (mIndex == 0) unpack_ix1_rad(pRG,ifr);
      if (mIndex == 1) unpack_ox1_rad(pRG,ifr);
      ierr = MPI_Waitany(2,recv_rq,&mIndex,MPI_STATUS_IGNORE);
      if (mIndex == 0) unpack_ix1_rad(pRG,ifr);
      if (mIndex == 1) unpack_ox1_rad(pRG,ifr);
...
/* MPI blocks to both left and right */
    if (pRG->rx2_id >= 0 && pRG->lx2_id >= 0) {
      ...
      /* check non-blocking receives and unpack data in any order. */
      ierr = MPI_Waitany(2,recv_rq,&mIndex,MPI_STATUS_IGNORE);
      if (mIndex == 0) unpack_ix2_rad(pRG,ifr);
      if (mIndex == 1) unpack_ox2_rad(pRG,ifr);
      ierr = MPI_Waitany(2,recv_rq,&mIndex,MPI_STATUS_IGNORE);
      if (mIndex == 0) unpack_ix2_rad(pRG,ifr);
      if (mIndex == 1) unpack_ox2_rad(pRG,ifr);
...
/* MPI blocks to both left and right */
    if (pRG->rx3_id >= 0 && pRG->lx3_id >= 0) {
      ...
      /* check non-blocking receives and unpack data in any order. */
      ierr = MPI_Waitany(2,recv_rq,&mIndex,MPI_STATUS_IGNORE);
      if (mIndex == 0) unpack_ix3_rad(pRG,ifr);
      if (mIndex == 1) unpack_ox3_rad(pRG,ifr);
      ierr = MPI_Waitany(2,recv_rq,&mIndex,MPI_STATUS_IGNORE);
      if (mIndex == 0) unpack_ix3_rad(pRG,ifr);
      if (mIndex == 1) unpack_ox3_rad(pRG,ifr);
...
/*----------------------------------------------------------------------------*/
/*! \fn void bvals_rad_init(MeshS *pM)
 *  Sets function pointers for physical boundaries during
...
/* Cycle through all the Domains that have active RadGrids on this proc */
  ...
  if (pM->Domain[nl][nd].Grid != NULL) {
    ...
    pRG = pM->Domain[nl][nd].RadGrid;          /* ptr to RadGrid */
    pROG = pM->Domain[nl][nd].RadOutGrid;          /* ptr to RadGrid */
    irefine = 1;
...
/* Domain is at L-edge of root Domain */
          ...
          case 4: /* Periodic. Handle with MPI calls for parallel jobs. */
            ...
            if (pROG != NULL)
              if(pROG->lx1_id < 0 && pD->NGrid[0] > 1){
                pROG->lx1_id=pD->GData[myN][myM][pD->NGrid[0]-1].ID_Comm_Domain;
              }
#endif /* MPI_PARALLEL */
            break;
...
/* Domain is at R-edge of root Domain */
          ...
          case 4: /* Periodic. Handle with MPI calls for parallel jobs. */
            ...
            if (pROG != NULL)
              if(pROG->rx1_id < 0 && pD->NGrid[0] > 1){
                pROG->rx1_id = pD->GData[myN][myM][0].ID_Comm_Domain;
              }
#endif /* MPI_PARALLEL */
            break;
...
/* Domain is at L-edge of root Domain */
          ...
          case 4: /* Periodic. Handle with MPI calls for parallel jobs. */
            ...
            if (pROG != NULL)
              if(pROG->lx2_id < 0 && pD->NGrid[1] > 1){
                pROG->lx2_id=pD->GData[myN][pD->NGrid[1]-1][myL].ID_Comm_Domain;
              }
#endif /* MPI_PARALLEL */
            break;
...
/* Domain is at R-edge of root Domain */
          ...
          case 4: /* Periodic. Handle with MPI calls for parallel jobs. */
            ...
            if (pROG != NULL)
              if(pROG->rx2_id < 0 && pD->NGrid[1] > 1){
                pROG->rx2_id = pD->GData[myN][0][myL].ID_Comm_Domain;
              }
#endif /* MPI_PARALLEL */
            break;
...
/* Domain is at L-edge of root Domain */
          ...
          case 4: /* Periodic. Handle with MPI calls for parallel jobs. */
            ...
            if (pROG != NULL)
              if(pROG->lx3_id < 0 && pD->NGrid[2] > 1){
                pROG->lx3_id=pD->GData[pD->NGrid[2]-1][myM][myL].ID_Comm_Domain;
              }
#endif /* MPI_PARALLEL */
            break;
...
/* Domain is at R-edge of root Domain */
          ...
          case 4: /* Periodic. Handle with MPI calls for parallel jobs. */
            ...
            if (pROG != NULL)
              if(pROG->rx3_id < 0 && pD->NGrid[2] > 1){
                pROG->rx3_id = pD->GData[0][myM][myL].ID_Comm_Domain;
              }
#endif /* MPI_PARALLEL */
            break;
...
/* Figure out largest size needed for send/receive buffers with MPI ----------*/
    ...
    if (radt_mode == 0) { /* integration only */
      noct = pRG->noct; nang = pRG->nang;
    } else if (radt_mode == 1) { /* output only */
      noct = pROG->noct; nang = pROG->nang;
...
/*----------------------------------------------------------------------------*/
/*! \fn void bvals_rad_trans_fun(DomainS *pD, enum BCDirection dir,
 *                               VRGIFun_t prob_bc)
 * Called from problem generator to specify user-defined boundary condition. */
...
/*----------------------------------------------------------------------------*/
/* PROLONGATION boundary conditions.  Does nothing.  Included for consistency
 * with bvals_mhd() calls  */
