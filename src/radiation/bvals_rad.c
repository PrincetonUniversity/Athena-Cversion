...
#ifdef MPI_PARALLEL
  int cnt, ierr, mIndex;
  int cnt0 = pRG->nf * (1 + pRG->nmu * pRG->ng);
  int cnt02 = pRG->nf * (1 + 2 * pRG->nmu * pRG->ng);
#endif /* MPI_PARALLEL */
  int l;
...
/* MPI blocks to both left and right */
    if (pRG->rx1_id >= 0 && pRG->lx1_id >= 0) {
      //printf("%d %d %d here\n",myID_Comm_world,pRG->lx1_id,pRG->rx1_id);
...
/* MPI blocks to both left and right */
    if (pRG->rx2_id >= 0 && pRG->lx2_id >= 0) {
      //printf("%d %d %d here\n",myID_Comm_world,pRG->lx2_id,pRG->rx2_id);
      /* Post non-blocking receives for data from L and R Grids */
...
/* Allocate memory for send/receive buffers and MPI_Requests */
  ...
  size *= pRG->nf * (1 + 3.0 * pRG->nmu * pRG->ng);
...
static void periodic_ix1_rad(RadGridS *pRG)
{
  ...
  int r1ls = pRG->r1ls, r1le = pRG->r1le;
  int r1ms = pRG->r1ms, r1me = pRG->r1me;
  int r2ls = pRG->r2ls, r2le = pRG->r2le;
  int r2ms = pRG->r2ms, r2me = pRG->r2me;
  int l2ls = pRG->l2ls, l2le = pRG->l2le;
  int l2ms = pRG->l2ms, l2me = pRG->l2me; 
  int nf = pRG->nf;
  int j,k,l,m,ifr;
  static count = 0;
  ...
  for (k=kl; k<=ku; k++){
	...
	for(l=r1ls; l<=r1le; l++)
	  for(m=r1ms; m<=r1me; m++) {
	    pRG->l1imu[k][j][ifr][l][m] = pRG->r1imu[k][j][ifr][l][m];	
	  }
      }}}
...
static void periodic_ox1_rad(RadGridS *pRG)
{
  ...
  int jl = pRG->js, ju = pRG->je;
  int kl = pRG->ks, ku = pRG->ke;
  int l1ls = pRG->l1ls, l1le = pRG->l1le;
  int l1ms = pRG->l1ms, l1me = pRG->l1me; 
  int r2ls = pRG->r2ls, r2le = pRG->r2le;
  int r2ms = pRG->r2ms, r2me = pRG->r2me;
  int l2ls = pRG->l2ls, l2le = pRG->l2le;
  int l2ms = pRG->l2ms, l2me = pRG->l2me; 
...
static void periodic_ix2_rad(RadGridS *pRG)
{
  ...
  for (k=kl; k<=ku; k++) {
	...
	for(l=r2ls; l<=r2le; l++)
	  for(m=r2ms; m<=r2me; m++) 
	    for(n=0; n<2; n++)
	      pRG->l2imu[k][i][ifr][l][m][n] = pRG->r2imu[k][i][ifr][l][m][n];
      }}}
  ...
  /*for (k=kl; k<=ku; k++){
	...
	for(l=r1ls; l<=r1le; l++)
	  for(m=r1ms; m<=r1me; m++)
	    pRG->l1imu[k][jl][ifr][l][m] = pRG->r1imu[k][je][ifr][l][m];
      }}
  ...
  for (k=kl; k<=ku; k++){
	...
	for(l=l1ls; l<=l1le; l++)
	  for(m=l1ms; m<=l1me; m++)
	    pRG->r1imu[k][jl][ifr][l][m] = pRG->l1imu[k][je][ifr][l][m];
	    }}*/
...
static void periodic_ox2_rad(RadGridS *pRG)
{
  ...
  for (k=kl; k<=ku; k++) {
	...
	for(l=l2ls; l<=l2le; l++)
	  for(m=l2ms; m<=l2me; m++)
	    for(n=0; n<2; n++)
	      pRG->r2imu[k][i][ifr][l][m][n] = pRG->l2imu[k][i][ifr][l][m][n];
      }}}
  ...
  /*for (k=kl; k<=ku; k++){
	...
	for(l=r1ls; l<=r1le; l++)
	  for(m=r1ms; m<=r1me; m++)
	    pRG->l1imu[k][ju][ifr][l][m] = pRG->r1imu[k][js][ifr][l][m];
      }}
  ...
  for (k=kl; k<=ku; k++){
	...
	for(l=l1ls; l<=l1le; l++)
	  for(m=l1ms; m<=l1me; m++)
	    pRG->r1imu[k][ju][ifr][l][m] = pRG->l1imu[k][js][ifr][l][m];
	    }}*/
...
static void unpack_ix2_rad(RadGridS *pRG)
{
  ...
  for (k=kl; k<=ku; k++) {
	...
	for(l=r2ls; l<=r2le; l++)
	  for(m=r2ms; m<=r2me; m++) 
	    for(n=0; n<2; n++) {
	      pRG->l2imu[k][i][ifr][l][m][n] = *(pRcv++);
	    //if((myID_Comm_world==1) && (i < 10))
	    //printf("iu %d %d %d %d %g\n",count,i,l,m,pRG->l2imu[k][i][ifr][l][m]);
	  }
      }}}
...
static void unpack_ox2_rad(RadGridS *pRG)
{
  ...
  for (k=kl; k<=ku; k++) {
	...
	for(l=l2ls; l<=l2le; l++)
	  for(m=l2ms; m<=l2me; m++) 
	    for(n=0; n<2; n++) {
	      pRG->r2imu[k][i][ifr][l][m][n] = *(pRcv++);
	    //if((myID_Comm_world==0) && (i < 10))
	    // printf("ou %d %d %d %d %g\n",count,i,l,m,pRG->r2imu[k][i][ifr][l][m]);
	  }
      }}}
