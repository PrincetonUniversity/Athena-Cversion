...
/* Initialize RadGrid used for integration of the hydrodynamics */
        if (radt_mode == 0) { /* integration only */
          ...
          ath_error("[init_radiation]: radiation mode must be 0,1, or 2 "
                    "but is set to %d\n");
        }
...
/* set function pointer to appropriate initalization routine based on
 * dimensions */
  if (dim == 1)
...
/* Allocate memory for intensities, angles and weights for angular
 * quadratures */
