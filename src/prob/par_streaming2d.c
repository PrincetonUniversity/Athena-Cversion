 ...
 *   This code works in 2D ONLY. Isothermal eos is assumed, and the value etavk/iso_sound
 *   is fixed. In the case of growth rate test, since one requires an integer number of
 *   wavelengths across the grid, the isothermal sound speed need to be re-evaluated.
 *
 * Perturbation modes:
...
void problem(Grid *pGrid, Domain *pDomain)
{
  ...
  if (par_geti("grid","Nx2") == 1) {
    ath_error("[par_streaming2d]: streaming2D only works for 2D problem.\n");
  }
...
/* Initialize boxsize */
  ...
  Nx = pGrid->Nx1; /* used for particle selection function */
...
/* Read initial conditions */
  ...
  if (par_geti("particle","partypes") != 1)
    ath_error("[par_streaming2d]: This test works only for ONE particle species!\n");
  ...
  mratio = 0.0;
  if ((ipert == 1) || (ipert == 2))
    ath_error("[par_streaming2d]: linear growth test requires FEEDBACK to be turned on!\n");
#endif
  amp = amp*mratio; /* N.B.! */
  ...
  {
    if (Lx != Lz)
      ath_error("[par_streaming2d]: Linear test requires Lx1=Lx2!\n");
    ...
    if (cs != Iso_csound)
      ath_pout(0, "[par_streaming2d]: Iso_csound=%f is modified to cs=%f.\n", Iso_csound, cs);
    Iso_csound = cs;
...
//              x2p = x2u + pGrid->dx2*ran2(&iseed);
            ...
            if ((ipert == 1) || (ipert == 2)) {
              pGrid->particle[p].x1 += paramp*cos(kz*x2p)*(-sin(kx*x1p)+factor2*paramp*sin(2.0*kx*x1p))/kx;
//              pGrid->particle[p].x1 += amp*cos(kz*x2p)*(-sin(kx*x1p)+0.5*amp*sin(2.0*kx*x1p))/kx;
...
/* flush output file */
  sprintf(name, "%s_%d_%d.dat", pGrid->outfilename,pGrid->Nx1,ipert);
