...
{
  int i=0,j=0,k=0;
  int is,ie,js,je,ks,ke,n,m,nx1,nx2,nx3,Nx1,Nx2,Nx3;
  Real amp,vflow;
...
/* allocate memory for solution and vector potential */
  ...
  if ((a1 = (Real***)calloc_3d_array(nx3, nx2, nx1, sizeof(Real))) == NULL)
    ath_error("[linear_wave3d]: Error allocating memory for \"a1\"\n");

  if ((a2 = (Real***)calloc_3d_array(nx3, nx2, nx1, sizeof(Real))) == NULL)
    ath_error("[linear_wave3d]: Error allocating memory for \"a2\"\n");
...
/* The file exists -- reopen the file in append mode */
  if((fp=fopen(fname,"r")) != NULL){
    if((fp = freopen(fname,"a",fp)) == NULL){
      fprintf(stderr,"[Userwork_after_loop]: Unable to reopen file.\n");
      free(fname);
      return;
    }
  }
