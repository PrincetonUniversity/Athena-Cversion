...
void problem(Grid *pGrid, Domain *pDomain)
{
  ...
  if (par_geti("grid","Nx3") == 1) {
    ath_error("[par_streaming3d]: streaming3D only works for 3D problem.\n");
  }
...
/* Read initial conditions */
  ...
  if (mratio < 0.0)
    ath_error("[par_streaming3d]: mratio must be positive!\n");
#else
  mratio = 0.0;
  if ((ipert == 1) || (ipert == 2))
    ath_error("[par_streaming3d]: linear growth test requires FEEDBACK to be turned on!\n");
#endif
  amp = amp*mratio; /* N.B.! */
  ...
  {
    if (Lx != Lz)
      ath_error("[par_streaming3d]: Linear test requires Lx1=Lx2!\n");
    ...
    if (cs != Iso_csound)
      ath_pout(0, "[par_streaming3d]: Iso_csound=%f is modified to cs=%f.\n", Iso_csound, cs);
    Iso_csound = cs;
...
void OutputModeAmplitude(Grid *pGrid, Domain *pDomain, Output *pOut)
{
  ...
  if(err) ath_error("[par_streaming3d]: MPI_Reduce returned error code %d\n",err);
