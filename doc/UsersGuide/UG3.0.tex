...
it is better to edit the template file {\tt /athena3.0/src/Makefile.in}.
...
on the command line.  Currently, the options for several target machines are
included in {\tt Makefile.in}.  The default machine uses the {\tt gcc} compiler
with an optimzation level of {\tt -O3}.  By using
\begin{verbatim}
% make all MACHINE=ophir
\end{verbatim}
the Intel C compiler (icc) combined with the options
...
rename it, amd edit as apropriate. 
